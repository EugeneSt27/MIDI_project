...
- Обработка файла: `python main.py` — результаты в `results/`.
- Анализ: Графы показывают inheritance (например, фраза 2 наследует от 1 с весом 0.8).
- Визуализация: Откройте HTML в браузере для интерактивности.

## Для статьи

Проект демонстрирует:
- Инвариантность к транспозиции (относительный pitch).
- Многомерное сходство для точного inheritance.
- Граф как основу для генерации (расширение сегментов).
